...
import { validateUrl, validateUrlAsync, extractDomain, secureLookup, getSecureAxios } from '../url.helper';
...
describe('validateUrl', () => {
  ...
  it('throws on ::ffff:169.254.169.254 (mapped AWS metadata)', () => {
    expect(() => validateUrl('http://[::ffff:169.254.169.254]')).toThrow(AppError);
  });
...
describe('validateUrlAsync', () => {
  ...
  it('throws when dual-stack hostname has one private address among public ones', async () => {
    ...
    await expect(validateUrlAsync('https://mixed.attacker.com')).rejects.toThrow(AppError);
  });
  ...
  it('throws when DNS lookup fails', async () => {
    ...
    await expect(validateUrlAsync('https://nonexistent.invalid')).rejects.toThrow(AppError);
  });

  it('throws when DNS returns empty address list', async () => {
    mockLookup([]);
    await expect(validateUrlAsync('https://empty.invalid')).rejects.toThrow(AppError);
  });

  it('skips DNS lookup for bare IPv4 literal blocked by sync check', async () => {
  await expect(validateUrlAsync('http://127.0.0.1')).rejects.toThrow(AppError);
  ...
  await expect(validateUrlAsync('http://93.184.216.34')).resolves.toBeDefined();
  expect(dns.lookup).not.toHaveBeenCalled();
  });
});
...
describe('secureLookup', () => {
  ...
  it('calls callback with error when resolving to multiple addresses containing a private IP', (done) => {
    ...
    secureLookup('mixed.com', { all: true }, (err: any, address: any, family: any) => {
      expect(err).toBeDefined();
      expect(err!.message).toContain('Access to private IP');
      done();
    });
...
describe('secureAxios SSRF & Redirect validation', () => {
  it('throws error in beforeRedirect if host is a private IP literal', () => {
    const requestConfig = (getSecureAxios().interceptors.request as any).handlers[0];
