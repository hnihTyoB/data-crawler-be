...
describe("calcDataQualityScore", () => {
  ...
  it("returns null for failed pages (not success)", () => {
    expect(
      calcDataQualityScore({ ...baseParams, isSuccess: false }),
    ).toBeNull();
  });
...
describe("detectWarnings", () => {
  ...
  it("adds MISSING_TITLE when title is null", () => {
    expect(detectWarnings({ ...cleanPage, title: null })).toContain(
      "MISSING_TITLE",
    );
  });

  it("adds MISSING_DESCRIPTION when description is null", () => {
    expect(detectWarnings({ ...cleanPage, description: null })).toContain(
      "MISSING_DESCRIPTION",
    );
  });

  it("adds TOO_SHORT when wordCount < 100 and > 0", () => {
    expect(detectWarnings({ ...cleanPage, wordCount: 30 })).toContain(
      "TOO_SHORT",
    );
  });

  it("does NOT add TOO_SHORT when wordCount is 0 (empty page — different case)", () => {
    expect(detectWarnings({ ...cleanPage, wordCount: 0 })).not.toContain(
      "TOO_SHORT",
    );
  });
  ...
  it("adds DUPLICATE_CONTENT when isDuplicateContent is true", () => {
    expect(
      detectWarnings({ ...cleanPage, isDuplicateContent: true }),
    ).toContain("DUPLICATE_CONTENT");
  });

  it("adds NAV_NOISE when isNavNoise is true", () => {
    expect(detectWarnings({ ...cleanPage, isNavNoise: true })).toContain(
      "NAV_NOISE",
    );
  });

  it("adds LOW_QUALITY_SCORE when dataQualityScore is below threshold (30)", () => {
    expect(detectWarnings({ ...cleanPage, dataQualityScore: 20 })).toContain(
      "LOW_QUALITY_SCORE",
    );
  });
  ...
  it("does NOT add LOW_QUALITY_SCORE when dataQualityScore is null (failed page)", () => {
    expect(
      detectWarnings({ ...cleanPage, dataQualityScore: null }),
    ).not.toContain("LOW_QUALITY_SCORE");
  });
...
describe("normalizeUrl", () => {
  ...
  it("strips known UTM tracking params", () => {
    const url =
      "https://example.com/article?utm_source=google&utm_medium=cpc&id=123";
    expect(normalizeUrl(url)).toBe("https://example.com/article?id=123");
  });

  it("strips fbclid and gclid tracking params", () => {
    expect(normalizeUrl("https://example.com/?fbclid=abc&page=1")).toBe(
      "https://example.com/?page=1",
    );
    expect(normalizeUrl("https://example.com/?gclid=xyz&q=2")).toBe(
