...
): CrawlPage & { normalizedUrl?: string | null } => ({
...
const makeLinkAsset = (
  url: string,
  sourceUrl: string = "https://example.com/article/deep-dive",
): CrawlAsset => ({
  id: `link-${url}`,
...
describe("Regression: links extraction", () => {
  ...
  it("classifies internal link correctly", () => {
    ...
    const internal = links.find(
      (l) => l.url === "https://example.com/next-page",
    );
  ...
  it("classifies external link correctly", () => {
    ...
    const external = links.find(
      (l) => l.url === "https://external.com/resource",
    );
