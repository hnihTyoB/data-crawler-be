...
export const crawlPageQuerySchema = z.object({
  ...
  dataQualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  minDataQualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  maxDataQualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  qualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  minQualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  maxQualityScore: z.preprocess(parseNumberQuery, z.number().min(0).max(100).optional()),
  ...
  minContentLength: z.preprocess(parseNumberQuery, z.number().min(0).optional()),
  maxContentLength: z.preprocess(parseNumberQuery, z.number().min(0).optional()),
  ...
  limit: z.preprocess(parseNumberQuery, z.number().int().min(1).max(100).default(20)),
  sortBy: z
    .enum([
      'createdAt',
      'updatedAt',
