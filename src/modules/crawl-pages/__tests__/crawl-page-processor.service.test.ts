...
describe('CrawlPageProcessorService.inferStatus()', () => {
  ...
  it('returns CAPTCHA_DETECTED for captcha error', () => {
    expect(svc.inferStatus('captcha required', undefined)).toBe('CAPTCHA_DETECTED');
  });

  it('returns CAPTCHA_DETECTED for uppercase CAPTCHA', () => {
    expect(svc.inferStatus('CAPTCHA detected on page', undefined)).toBe('CAPTCHA_DETECTED');
  });

  // PAYWALL_DETECTED
  it('returns PAYWALL_DETECTED for paywall error', () => {
    expect(svc.inferStatus('paywall detected', undefined)).toBe('PAYWALL_DETECTED');
  });

  // REQUIRES_LOGIN
  it('returns REQUIRES_LOGIN for "requires login"', () => {
    expect(svc.inferStatus('page requires login', undefined)).toBe('REQUIRES_LOGIN');
  });

  it('returns REQUIRES_LOGIN for "login required"', () => {
    expect(svc.inferStatus('login required to view', undefined)).toBe('REQUIRES_LOGIN');
  });

  // TIMEOUT
  it('returns TIMEOUT for timeout error', () => {
    expect(svc.inferStatus('request timeout after 30s', undefined)).toBe('TIMEOUT');
  });
  ...
  it('returns BLOCKED for "too many requests"', () => {
    expect(svc.inferStatus('too many requests from this IP', undefined)).toBe('BLOCKED');
  });
  ...
  it('returns FAILED for apikey error', () => {
    expect(svc.inferStatus('invalid apikey provided', undefined)).toBe('FAILED');
  });

  it('returns FAILED for "forbidden" combined with "key"', () => {
    expect(svc.inferStatus('forbidden: invalid api key', undefined)).toBe('FAILED');
  });

  // FAILED — generic / unknown
  it('returns FAILED for DNS error', () => {
    expect(svc.inferStatus('dns enotfound example.com', undefined)).toBe('FAILED');
  });
