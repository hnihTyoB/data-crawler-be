...
describe("CrawlExportService findAllByUser and delete", () => {
  ...
  it("lists exports for authenticated user", async () => {
    ...
    (
      CrawlExportRepository.prototype.findAllByUser as jest.Mock
    ).mockResolvedValue({
      items: [mockExport],
      total: 1,
      page: 1,
  ...
  it("deletes export file from storage and database", async () => {
    ...
    (CrawlExportRepository.prototype.delete as jest.Mock).mockResolvedValue(
      mockExport,
    );
    ...
    expect(CrawlExportRepository.prototype.delete).toHaveBeenCalledWith(
      "exp-123",
    );
  });
});
